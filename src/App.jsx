...
import {
  ...
  MessageSquare, Users, Search, MoreVertical, X, ArrowLeft, Edit2, UserPlus, UserMinus, Trash, Info, Wallpaper, Ban, Lock, Unlock, Mail
} from "lucide-react";
import { initializeApp } from "firebase/app";
import {
  getFirestore, getDocs, collection, setDoc, addDoc, query, where, updateDoc, doc, onSnapshot, arrayUnion, arrayRemove, deleteDoc, orderBy, serverTimestamp
} from "firebase/firestore";
import { useAboutUs } from "./AboutUsContext";
import {
...
export function AboutUsProvider({ children }) {
  ...
  const [devGlowColor, setDevGlowColor] = useState("#00ff00");

  useEffect(() => {
    const ref = doc(db, "about", "app");
    const unsub = onSnapshot(ref, (docSnap) => {
      if (docSnap.exists()) {
        const d = docSnap.data();
        setAbout(d.about || "");
        setLogoURL(d.logoURL || "");
        setAboutPhotoURL(d.aboutPhotoURL || "");
        setDevGlowColor(d.devGlowColor || "#00ff00");
      }
    });
    return unsub;
  }, []);

  // Only developer calls this!
...
// ---------- ABOUTUS COMPONENT (uses AboutUsContext) ----------
const AboutUs = ({ onClose, canEdit, devAccount, onContact }) => {
  const {
    about, setAbout,
    logoURL, setLogoURL,
    aboutPhotoURL, setAboutPhotoURL,
  ...
  useEffect(() => { setLogoInput(logoURL); }, [logoURL]);
  useEffect(() => { setPhotoInput(aboutPhotoURL); }, [aboutPhotoURL]);

  async function handleSave() {
    await updateAboutUs({ about: localAbout });
    setEdit(false);
  }
  async function handleLogoUpdate() {
    await updateAboutUs({ logoURL: logoInput });
    setShowLogoInput(false);
  }
  async function handlePhotoUpdate() {
    await updateAboutUs({ aboutPhotoURL: photoInput });
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md w-full relative shadow-2xl">
        ...
        {edit ? (
          <>
            ...
            <div className="flex gap-2">
              <button className="flex-1 bg-blue-500 text-white rounded py-1" onClick={handleSave}>Save</button>
        ...
        {showLogoInput && canEdit && (
          <div className="mt-2 flex">
            ...
            <button className="ml-2 bg-blue-500 text-white px-2 py-1 rounded" onClick={handleLogoUpdate}>Update</button>
          </div>
        )}
        {showPhotoInput && canEdit && (
          <div className="mt-2 flex">
            ...
            <button className="ml-2 bg-blue-500 text-white px-2 py-1 rounded" onClick={handlePhotoUpdate}>Update</button>
          </div>
        )}
      </div>
    </div>
...
const Message = ({ message, currentUser, onEdit, onDelete }) => {
  ...
  const canDeleteForEveryone = () => {
    ...
    if (message.type === "image" || message.type === "text") {
      const now = Date.now();
      const ts = message.timestamp?.toMillis?.() || message.timestamp || 0;
          ...
          {isOwn && (
            <div className="flex items-center space-x-1">
              {isDelivered
                ...
                : <Check size={14} />}
              <button onClick={() => onDelete(message.id)} className="ml-1 text-red-400 hover:text-red-600" title="Delete">
                <Trash size={14} />
              </button>
              {canDeleteForEveryone() && (
                <button onClick={() => onDelete(message.id, true)} className="ml-1 text-red-600 font-bold" title="Delete for everyone">
                  <Trash2 size={14} />
                </button>
              )}
              {isOwn && (
                <button onClick={() => onEdit(message)} className="ml-1 text-blue-400 hover:text-blue-600" title="Edit">
                  <Edit2 size={14} />
                </button>
              )}
            </div>
...
const ChatList = ({ currentUser, onSelectChat, activeChat, chatGlowColors = {}, onShowRequests }) => {
  ...
  return (
    <div className="chat-list w-84 bg-white dark:bg-gray-800 border-r dark:border-gray-700 flex flex-col" style={{ display: "block" }}>
...
const FriendRequests = ({ currentUser }) => {
  ...
  // --- ChatWindow with mobile back button ---
const ChatWindow = ({
  currentUser,
  activeChat,
  globalTheme,
...
// --- Mobile-aware Chat Window wrapper ---
const MobileChatWindow = ({
  ...
  <div className="fixed inset-0 sm:static bg-white dark:bg-gray-900 z-50 flex flex-col h-full w-full">
    <div className="flex items-center px-2 py-2 bg-white dark:bg-gray-800 border-b dark:border-gray-700">
      <button
        className="mr-2 p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 sm:hidden"
        onClick={onBack}
      >
        <ArrowLeft size={20} />
      </button>
    ...
    <ChatWindow {...props} currentUser={currentUser} activeChat={activeChat} />
  </div>
);

// --- Enhanced ChatList for mobile toggle ---
...
// ---- MOBILE FEATURE SWITCHING LOGIC ----
...
//   // Only one feature open at a time on mobile!
//   // Possible values: "chats", "profile", "friends", "requests", "settings", "about", "blocked"
//   const [mobileFeature, setMobileFeature] = useState("chats");
...
//   useEffect(() => { localStorage.setItem("theme", globalTheme); }, [globalTheme]);
//   useEffect(() => { localStorage.setItem("wallpapers", JSON.stringify(wallpapers)); }, [wallpapers]);
//   useEffect(() => { localStorage.setItem("chat_passwords", JSON.stringify(chatPasswords)); }, [chatPasswords]);
//   useEffect(() => { localStorage.setItem("chat_glow_colors", JSON.stringify(chatGlowColors)); }, [chatGlowColors]);
//   useEffect(() => { ensureDeveloperAccount().then(setDevAccount); }, []);
//   useEffect(() => {
//     setTimeout(() => setLoading(false), 1000);
...
//   const handleContactUs = async () => {
//     if (!firebaseUser || !devAccount) return;
//     const q = query(collection(db, "friendRequests"), where("participants", "array-contains", firebaseUser.uid));
//     const snap = await getDocs(q);
//     if (snap.docs.some(doc => {
//       const data = doc.data();
//       return Array.isArray(data.participants) &&
...
//   ) : (
//     <Login
//       onLogin={() => setFirebaseUser(auth.currentUser)}
...
// if (!isMobile()) {
//   // Desktop: sidebar + chat list + chat window, as usual
//   return (
//     <div className="h-screen flex bg-gray-50 dark:bg-gray-900">
//       <Sidebar
//         user={firebaseUser}
//         onProfile={() => setMobileFeature("profile")}
//         onFriends={() => setMobileFeature("friends")}
//         onChats={() => setMobileFeature("chats")}
//         onRequests={() => setMobileFeature("requests")}
//         onBlocked={() => setMobileFeature("blocked")}
//         onSettings={() => setMobileFeature("settings")}
//         onLogout={handleLogout}
//         onAbout={() => setMobileFeature("about")}
//         selected={mobileFeature}
//         logoURL={logoURL}
//       />
...
//           <UserProfile user={firebaseUser} aboutPhotoURL={aboutPhotoURL} />
//         )}
//         {mobileFeature === "settings" && (
//           <Settings
//             onAbout={() => setMobileFeature("about")}
//             onLogout={handleLogout}
//             chatPasswords={chatPasswords}
...
//         {mobileFeature === "about" && (
//           <AboutUs
//             onClose={() => setMobileFeature("chats")}
//             canEdit={canEditAbout}
//             devAccount={devAccount}
//             onContact={handleContactUs}
//           />
//         )}
//         {mobileFeature === "blocked" && (
//           <BlockedUsers currentUser={firebaseUser} onClose={() => setMobileFeature("chats")} />
//         )}
//       </div>
...
// // ------ MOBILE: Only one feature at a time ------
// return (
...
//       />
//     )}
//     {mobileFeature === "chatwindow" && activeChat && (
//       <MobileChatWindow
//         currentUser={firebaseUser}
//         activeChat={activeChat}
//         globalTheme={globalTheme}
...
//           />
//         )}
//         {showMobileChat && isMobile() && activeChat && (
//           <MobileChatWindow
//             currentUser={firebaseUser}
//             activeChat={activeChat}
//             globalTheme={globalTheme}
...
const App = () => {
  ...
  // Only one feature open at a time on mobile!
  // Possible values: "chats", "profile", "friends", "requests", "settings", "about", "blocked", "chatwindow"
  const [mobileFeature, setMobileFeature] = useState("chats");
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-gray-900 relative">
      {/* Hamburger button */}
      <button
        className="absolute top-4 left-4 z-50 p-2 bg-white dark:bg-gray-800 rounded-md shadow-md md:hidden"
      ...
      {mobileSidebarOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex">
          <div className="bg-white dark:bg-gray-900 w-60 h-full shadow-lg relative">
            <button
            ...
            <Sidebar
              user={firebaseUser}
              onProfile={() => { setMobileFeature("profile"); setMobileSidebarOpen(false); }}
              onFriends={() => { setMobileFeature("friends"); setMobileSidebarOpen(false); }}
              onChats={() => { setMobileFeature("chats"); setMobileSidebarOpen(false); }}
              onRequests={() => { setMobileFeature("requests"); setMobileSidebarOpen(false); }}
              onBlocked={() => { setMobileFeature("blocked"); setMobileSidebarOpen(false); }}
              onSettings={() => { setMobileFeature("settings"); setMobileSidebarOpen(false); }}
              onLogout={handleLogout}
              onAbout={() => { setMobileFeature("about"); setMobileSidebarOpen(false); }}
              selected={mobileFeature}
              logoURL={logoURL}
            />
          </div>
      ...
      {/* MAIN MOBILE CONTENT */}
      <div className={mobileSidebarOpen ? "opacity-20 pointer-events-none" : ""}>
        {mobileFeature === "chats" && (
          <ChatList
            currentUser={firebaseUser}
            onSelectChat={handleSelectChat}
