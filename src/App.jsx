...
export default function App() {
  ...
  const [newImage, setNewImage] = useState(null); // for sending image
  ...
  // Settings UI (combine all options inside Settings)
  ...
  // Handle file selection and preview for profile photo
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (profilePhotoURL) {
        try {
          // The url is a firebase download url, get the path after "/o/"
  ...
  async function sendMessage() {
    if ((!newMessage.trim() && !newImage) || !currentChatFriend || !user) return;
    const chatId = generateChatId(user.uid, currentChatFriend.uid);
    const messagesRef = collection(db, "chats", chatId, "messages");
    ...
    if (newImage) {
      ...
      try {
        const imgRef = ref(storage, `chatImages/${chatId}/${Date.now()}_${newImage.name}`);
          ...
          <div className="sidebar" style={{
            ...
            <div style={{
              ...
              <div style={{ position: 'relative', marginRight: 15 }}>
                <img
                  src={profilePhotoURL || "/images/About.jpg"}
                  alt="Profile"
                  style={{ width: 60, height: 60, borderRadius: "50%", border: '3px solid #219653', background: "#fff", objectFit: "cover" }}
                />
                <div style={{
                  position: 'absolute',
                  bottom: 0,
            ...
            {/* Settings Button - All options combined inside modal */}
                ...
                friendRequests.map((req) => (
                  <div key={req.id} style={{
                    ...
                    <div style={{ display: 'flex', gap: '5px' }}>
                      <button
                        onClick={() => {
                          acceptFriendRequest(req.id, req.from);
                          if (isMobile) setShowSidebar(false);
                        }}
                        style={{
                          flex: 1,
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{
                    padding: '12px 20px',
                    backgroundColor: (newMessage.trim() || newImage) ? '#219653' : '#ccc',
                    color: 'white',
                    border: 'none',
                    borderRadius: '25px',
                    cursor: (newMessage.trim() || newImage) ? 'pointer' : 'not-allowed',
                    fontSize: '14px'
                  }}
                >
                  Send
                ...
                {newImage && (
                  <div style={{ marginLeft: 6, position: "relative" }}>
                    <img src={URL.createObjectURL(newImage)} alt="preview" style={{ width: 30, height: 30, borderRadius: 6, objectFit: "cover" }} />
                    <span
        ...
        {showSettings && (
          ...
          >
            <div style={{
              ...
              {/* Blocked users */}
              <button
                onClick={() => { setShowBlockedUsers(!showBlockedUsers); setShowSettings(false); }}
                style={{
                  width: '100%',
                  padding: '8px',
                  backgroundColor: '#dc3545',
...
function WallpaperMenu({ chatWallpaper, handleChatWallpaperChange, removeChatWallpaper }) {
